...
type Plugin interface {
	...
	Run(*map[string]string, *map[string]string)
}
...
// it as arguments.
func runPlugin(name string, inputsMap, resultsMap *map[string]string) {
	// module path
...
func RunIfEnabled(pluginName string, inputsMap, resultsMap *map[string]string) bool {
	// only run the plugin if plugin_enabled != "false"
	if (*inputsMap)["plugin_enabled"] != "false" {
		runPlugin(pluginName, inputsMap, resultsMap)
