...
func (p plugin) Run(inputsMap, resultsMap *map[string]cty.Value) {
	...
	cmdName := "python3"
	driverPath := "/usr/bin/Sublist3r/sublist3r.py"
	cmdArgs := []string{driverPath, "-d", targetHost}

	if cmdOut, err = exec.Command(cmdName, cmdArgs...).Output(); err != nil {
		fmt.Fprintln(os.Stderr, "There was an error running the sublist3r tool: ", err)
		return
	}
