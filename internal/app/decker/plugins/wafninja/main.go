...
func (p plugin) Run(inputsMap, resultsMap *map[string]string, resultsListMap *map[string][]string) {
	...
	cmdName := "python"
	cmdArgs := []string{"/usr/bin/WAFNinja/wafninja.py", "bypass", "-u", "http://" + targetHost + "/index.php", "-p", "Name=PAYLOAD&Submit=Submit",
		"-c", "phpsessid=value", "-t", "xss", "-o", "output.html"}

	if cmdOut, err = exec.Command(cmdName, cmdArgs...).Output(); err != nil {
		fmt.Fprintln(os.Stderr, "There was an error running wafninja command: ", err)
		return
	}
